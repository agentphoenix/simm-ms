...
('2.6.9', '1234400160', 'This release fixes bugs with the docking request form, docked ship activation and database entry management.', 'Fixed typos in docking request email sent out to the starbase CO;Fixed bug with docked ship activation and rejection where the docking CO wouldn\'t be sent a copy of the acceptance or rejection email;Fixed location of the Facebox loading graphic;Fixed bug in database management page where only entries with a display flag of yes would be shown instead of all entries;Fixed bug in database display page where departments with database use turned off still appeared;Updated the version check class to understand that SMS 3.0 is actually Nova 1.0;Updated the version check class to have the download link point to the main Anodyne site;Updated the version check class to point to a new XML file so Nova and SMS can both have version XML files with the same naming scheme')");
