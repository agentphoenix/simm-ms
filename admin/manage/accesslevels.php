...
if( in_array( "x_access", $sessionAccess ) ) {
	...
	if(
		...
		/* update the database */
		$update = "UPDATE sms_accesslevels SET $type = '$accessString' WHERE id = $id LIMIT 1";
...
?>
	...
	<div class="body">
		...
		<?
			...
			$crewAccessRaw = $fetchAccess[1] . "," . $fetchAccess[2] . "," . $fetchAccess[3] . "," . $fetchAccess[4] . "," . $fetchAccess[5];
			$crewAccess = explode( ",", $crewAccessRaw );
	
			/* query check */
			$check = new QueryCheck;
			$check->checkQuery( $result, $update );
