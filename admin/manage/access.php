...
Last Modified: 2007-11-07 0835 EST
**/
...
if( in_array( "x_access", $sessionAccess ) ) {
	...
	/* do some logic to make sure we're touching the right db table */
	if( $sec == "post" ) {
		$table = "accessPost";
	} elseif( $sec == "manage" ) {
		$table = "accessManage";
	...
	if( $action ) {
		...
		/* pop the update button variable off the end of the array */
		array_pop( $accessValues );

		/* pop the update button x variable off the end of the array */
		array_pop( $accessValues );

		/* pop the update button y variable off the end of the array */
		array_pop( $accessValues );
		...
		/* update the database */
		$updateAccess = "UPDATE sms_crew SET $table = '$accessString' WHERE crewid = '$crew' LIMIT 1";
		...
		/* do the update query */
		$updateAccess = "UPDATE sms_crew SET accessPost = '$levelsPost', accessManage = '$levelsManage', ";
		$updateAccess.= "accessReports = '$levelsReports', accessUser = '$levelsUser', ";
		$updateAccess.= "accessOthers = '$levelsOther' WHERE crewid = '$crew' LIMIT 1";
		$result = mysql_query( $updateAccess );
	...
	$getCrewAccess = "SELECT $table FROM sms_crew WHERE crewid = '$crew' LIMIT 1";
	$getCrewAccessResult = mysql_query( $getCrewAccess );
	$fetchAccess = mysql_fetch_array( $getCrewAccessResult );
	$crewAccess = explode( ",", $fetchAccess['0'] );
...
?>
	...
	<div class="body">
		...
		while( $userFetch = mysql_fetch_assoc( $getCrewResult ) ) {
			...
			echo "<a href='" . $webLocation . "admin.php?page=manage&sub=access&crew=" . $userFetch['crewid'] . "'>" . $userFetch['rankName'] . " " . $userFetch['firstName'] . " " . $userFetch['lastName'] . "</a><br />";
		...
		<ul class="list-dark">
			<li><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=<?=$sec;?>&set=co">Grant CO Access Levels</a></li>
			<li><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=<?=$sec;?>&set=xo">Grant XO Access Levels</a></li>
			<li><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=<?=$sec;?>&set=dh">Grant Department Head Access Levels</a></li>
			<li><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=<?=$sec;?>&set=player">Grant Standard Access Levels</a></li>
		</ul>
		...
		<div id="subnav">
			<ul>
				<li <? if( $sec == "post" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=post">Post</a></li>
				<li <? if( $sec == "manage" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=manage">Manage</a></li>
				<li <? if( $sec == "reports" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=reports">Reports</a></li>
				<li <? if( $sec == "user" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=user">User</a></li>
				<li <? if( $sec == "other" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=other">Other</a></li>
			</ul>
		</div>
		...
		<? if( $sec == "post" ) { ?>
		<div class="pmHeader">
		<form method="post" action="<?=$webLocation;?>admin.php?page=manage&sub=access&crew=<?=$crew;?>&sec=<?=$sec;?>">
		...
		<? accessControls( $webLocation, $sessionDisplaySkin ); ?>
		</form>
