...
Last Modified: 2007-08-02 1438 EST
**/
...
<div class="body">
	...
	if(
		...
		checkPendings( $sessionAccess, $simmType );
	}
	
	/* check for unread messages */
	if( in_array( "u_inbox", $sessionAccess ) ) {
		checkUnreadMessages( $sessionCrewid );
	}
	...
	if( $usePosting == "y" &&
		...
		checkSavedPosts( $sessionCrewid, $sessionAccess );
	}
	...
	<div id="subnav">
		<ul>
			...
			<? if( $cpShowPosts == "y" ) { ?>
			<li <? if( $sec == "posts" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=main&sec=posts">Recent Mission Posts</a></li>
			<? } ?>

			<? if( $cpShowLogs == "y" ) { ?>
			<li <? if( $sec == "logs" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=main&sec=logs">Recent Personal Logs</a></li>
			<? } ?>

			<? } ?>

			<? if( $cpShowNews == "y" ) { ?>
			<li <? if( $sec == "news" ) { echo "id='current'"; } ?>><a href="<?=$webLocation;?>admin.php?page=main&sec=news">Recent News Items</a></li>
			<? } ?>
		</ul>
	</div>
	...
	<? if( $usePosting == "y" && $cpShowPosts == "y" && $sec == "posts" ) { ?>
	<br />
	<!-- recent posts -->
		<?
	...
	<? if( $usePosting == "y" && $cpShowLogs == "y" && $sec == "logs" ) { ?>
	<br />
	...
	<!-- recent news items -->
		...
		<span class="fontSmall">
			Posted by <? printCrewName( $newsItems['newsAuthor'], "rank", "link" ); ?> on <?=dateFormat( "long", $newsPosted );?><br />
	...
	<? } /* close the logic that determines if users want to see stuff on the CP */ ?>
</div>